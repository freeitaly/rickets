...
from backtestBase import Strategy, Portfolio
...
def runBacktesting():
    ...
    # on the command line)
    import pymongo
    client = pymongo.MongoClient('10.10.40.20:28019')
    dbName = 'VnTrader_1Min_Db'
    symbol = 'rb0000'

    collection = client[dbName][symbol]

    bars = pd.DataFrame(list(collection.find({'date':{"$gte":'20170401'}})))
    print bars

    # Create a set of random forecasting signals for SPY
    rfs = RandomStrategy(symbol, bars)
