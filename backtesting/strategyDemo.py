...
class DemoStrategy(Strategy):
    ...
    def __init__(self, engine, setting):
        ...
        self.para2 = setting['para2']
        self.para3 = setting['para3']
    ...
    def loadHistoryData(self):
        ...
        # 原始数据===>策略使用数据
        # 生成15min(self.period='15min') K线
        # bars = bars.resample('self.period').last().dropna()  # 需要去除NA值
    ...
    def generate_signals(self):
        ...
        # 由bars报价数据生成信号计算所需的技术指标
        # <editor-fold desc="Demo: 生成短期MA和长期MA指标">
        ...
        # <editor-fold desc="Demo: 由长短MA交叉，生成交易信号">
        ...
        # 产生交易信号, signals数据中，写入close、positions等绩效计算所需数据
        # Take the difference of the signals in order to generate actual trading orders
        # <editor-fold desc="Demo: signals数据中，写入close、positions等绩效计算所需数据">
...
def backtesting():
    ...
    setting.addParameter('para2', 6, 10, 1)  # 增加第一个优化参数atrLength，起始11，结束12，步进1
    setting.addParameter('para3', 18, 30, 1)  # 增加第二个优化参数atrMa，起始20，结束30，步进1

    # 执行回测、单线程优化或者多线程优化(三选一）
    # runBacktesting(engine)
    # runOptimization(engine, DemoStrategy, setting)
    # runParallelOptimization(engine, DemoStrategy, MarketOnClosePortfolio, setting)
    runBacktesting(engine)
